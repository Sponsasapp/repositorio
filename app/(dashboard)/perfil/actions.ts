...
import { MODALITY_VALUES } from "@/lib/sports";
...
export async function salvarPerfilPiloto(
  ...
  const pkgRows = parsePackages(formData.get("packages"), user.id, modality);
  ...
  // 2. athlete_modalities — a parte esportiva da modalidade ativa (upsert)
  const { error: e2 } = await supabase
    ...
    .upsert(
      ...
      { onConflict: "profile_id,modality" },
    );
  if (e2) return { error: "Não foi possível salvar os dados esportivos." };

  // 2b. Carros + conquistas desta modalidade — substitui tudo.
  ...
  // 4. athlete_packages — tabela de preços desta modalidade (substitui tudo)
  await supabase
...
export async function salvarPerfilEmpresa(
  ...
  const modalities = formData
    .getAll("modalities")
    .map(String)
    .filter((m) => MODALITY_VALUES.includes(m));
